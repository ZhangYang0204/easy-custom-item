...
public class RemoveItemStackItemFlagExecutor extends ExecutorBase {
    public RemoveItemStackItemFlagExecutor(@NotNull CommandSender sender, String commandName, @NotNull String[] args) {
    ...
    protected void run() {
        ...
        if (!itemMeta.getItemFlags().contains(itemFlag)) {
            List<String> list = MessageYaml.INSTANCE.getStringList("message.chat.notExistItemFlag");
            MessageUtil.sendMessageTo(sender, list);
            return;
        }

        itemMeta.getItemFlags().remove(itemFlag);
        itemStack.setItemMeta(itemMeta);


        List<String> list = MessageYaml.INSTANCE.getStringList("message.chat.removeItemStackItemFlag");
        MessageUtil.sendMessageTo(sender, list);
    }
}
